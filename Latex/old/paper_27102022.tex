...
\end{align*}
Now if we apply the adjoint operator of the Malliavin derivative (see Elisa-David) we get

\begin{align*}
&CA_{CMS}(T_a,T_b)  \\
&=01(0,T_a,T_b) \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( \int_{0}^{T_a} \partial_{x}S_{a,b}(T_a) D_s x_{T_a} \mathbb{E}_s^{01(a,b)}\left(f^{\prime}(S^{ois}_{a,b}(T_a)) \partial_{x}S^{ois}_{a,b}(T_a) D_s x_{T_a} \right) ds \right) \nonumber \\
&\approx 01(0,T_a,T_b)  \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( \int_{0}^{T_a} \partial_{x}S_{a,b}(T_a, x_0, y_0) \partial_{x}S^{ois}_{a,b}(T_a, x_0, y_0) f^{\prime}(S^{ois}_{a,b}(T_a, x_0, y_0)  D_s x_{T_a} \mathbb{E}_s^{01(a,b)}\left(D_s x_{T_a} \right) ds \right) \nonumber \\
& \approx 01(0,T_a,T_b)  \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( \int_{0}^{T_a} \partial_{x}S_{a,b}(T_a, x_0, y_0) \partial_{x}S^{ois}_{a,b}(T_a, x_0, y_0) f^{\prime}(S^{ois}_{a,b}(T_a, x_0, y_0) \beta(s,T_a,x_0,\hat{y}_{s})  D_s x_{T_a}  ds \right)
\end{align*}
...
\begin{equation*}
\mathbb{E}_s^{\mathbb{Q}^{01(a,b)}}\left( D_s x_{T_a} \right) \approx \beta(s,T_a,x_0,\hat{y}_{s}) \exp\left(-\int_{s}^{T_a} \frac{\int_{T_a}^{T_b} P_{ois}(u,u^{\prime},x_0,\hat{y}_u) \nu(t,u^{\prime}) du^{\prime}}{01^{c}(u,T_a,T_b,x_0,\hat{y}_u)} \partial_x \beta(u,t_0,x_0,\hat{y}_u) du \right)
\end{equation*}
...
\begin{align}\label{cms_adjustment}
&CA_{CMS}(T_a,T_b) \nonumber \\
&\approx 01(0,T_a,T_b) \int_{0}^{T_a} \partial_{x}S_{a,b}(T_a, x_0, y_0) \partial_{x}S^{ois}_{a,b}(T_a, x_0, y_0)  f^{\prime}(S^{ois}_{a,b}(T_a, x_0, y_0)) \beta^{2}(s,T_a,x_0,\hat{y}_{s}) \Omega(s, T_a)    ds
\end{align}
with 
$$\Omega(s, T_a) = \exp\left(-2\int_{s}^{T_a} \frac{\int_{T_a}^{T_b} P_{ois}(u,u^{\prime},x_0,\hat{y}_u) \nu(t,u^{\prime}) du^{\prime}}{01^{c}(u,T_a,T_b,x_0,\hat{y}_u)} \partial_x \beta(u,t_0,x_0,\hat{y}_u) du \right) 
$$
}}
\begin{example}
...
\begin{equation}\label{cms_general_convexity}
01(0,T_a,T_b) \int_{0}^{T_a} \partial_{x}S_{a,b}(T_a,x_0,y_0) \partial_{x}S^{ois}_{a,b}(T_a,x_0,y_0) f^{\prime}(S^{ois}_{a,b}(T_a,x_0,y_0)) \beta^{2}(s,T_a,x_0,\hat{y}_{s}) ds
\end{equation}
...
Therefore
\begin{equation*}
\beta = \frac{P_ois(0,T_p)}{01(0,T_a,T_b,x_0,y_0)} - \alpha S^{ois}_{a,b}(0,x_0,y_0).
\end{equation*}
Under a separable HJM dynamic we must have that
\begin{equation*}
\alpha \partial_{x} S^{ois}_{a,b}(T_a,x_0,y_0) = - M(0,T_p)\left(G(T_a,T_p)+\frac{\partial_x 01(T_a,T_a,T_b,x_0,y_0)}{01(T_a,T_a,T_b,x_0,y_0)}\right).
\end{equation*}
Then
\begin{equation*}
\alpha = -\frac{M(T_a,T_p)\left(G(T_a,T_p)+\frac{\partial_x 01(T_a,T_a,T_b,x_0,y_0)}{01(T_a,T_a,T_b,x_0,y_0)}\right)}{\partial_{x} S^{ois}_{a,b}(T_a,x_0,y_0)}.
\end{equation*}
Then from the last equalities, we can approximate the convexity adjustment of a CMS rate as
\begin{align*}
CA_{CMS}(T_a,T_b)\\
\approx - P_{ois}(0,T_p,x_0,y_0) \left(G(T_a,T_p) +\frac{\partial_x 01(T_a,T_a,T_b,x_0,y_0)}{01(T_a,T_a,T_b,x_0,y_0)}\right) \partial_{x}S_{a,b}(0,x_0,y_0) \int_{0}^{T_a}  \beta^{2}(s,T_a,x_0,\hat{y}_{u}) ds
\end{align*}
