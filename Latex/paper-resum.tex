...
\bar{R}(t_0,t_1) \approx \frac{\exp\left(\mathbb{E}_t^{\mathbb{Q}}\left(I(t_0,t_1)\right)\right)\exp\left(-\int_{t}^{t_1}\frac{\Gamma^{2}(s,t_0,t_1)}{2} ds\right) - 1}{\delta_{t_0,t_1}}
\end{equation}
...
\end{align}
with  $\beta(u,t_a,x,y) = \exp\left(-\int_{u}^{t_a}k_u du\right)\eta(u,x,y)$.
\end{theorem}
\begin{proof}
...
D_s \bar{x}_{t_a} = \beta(s,t_a,\bar{x}_s,\bar{y}_s) \bar{M}(s,t_a)
\end{equation}
where
\begin{align*}
...
\end{align*}
With $\beta(u,t_a,x,y) = \exp\left(-\int_{u}^{t_a}k_u^{\prime} du^{\prime}\right) \partial_x \eta(u,x,y)$. Then, we have that
...
\end{equation*}
where $\bar{\nu}(t,t_p) = \int_{t}^{t_p} \eta(s,\bar{x}_s,\bar{y}_s) ds$. We must observe, that we can approximate $x_s$ by 
$$
\mathbb{E}^{t_p}(x_s) = \bar{x}_0(t_a) + \mathcal{O}\left(\int_{0}^{s}\exp\left(-\int_{u}^{t_a}k_u^{\prime} du^{\prime}\right) du \right)
$$. Let define
\begin{align*}
\bar{\nu}(t,t_p, \bar{x}_0(t_a))&:= \int_{t}^{t_p} \eta(s,\bar{x}_0(t_a),\bar{y}_s) ds\\
\partial_x \bar{\nu}(t,t_p, \bar{x}_0(t_a))&:= \int_{t}^{t_p} \partial_x \eta(s,\bar{x}_0(t_a),\bar{y}_s) ds\\
\end{align*}
...
\begin{align*}
\mathbb{E}_s(D_s \bar{x}_{t_a}) &\approx \eta(s,\bar{x}_s,\bar{y}_s) \exp\left(-\int_{s}^{t_a}k_u du \right)\\ &\cdot \mathbb{E}^{t_p}_s\left(\exp\left(-\int_{s}^{t_a} \exp\left(-\int_{u}^{t_a}k_u du\right)\left(\partial_x (\eta(u, \bar{x}_0(t_a), \bar{y}_{u})) \bar{\nu}(u,t_p, \bar{x}_0(t_a)) \right. \right. \right. \\
&+ \left. \left. \left. \eta(u, \bar{x}_0(t_a), \bar{y}_{u})) \partial_x  \bar{\nu}(t,t_p, \bar{x}_0(t_a)) du \right)\right)\right) 
\end{align*}
\end{proof}
...
In this case, we will choose $\bar{x}_0(t_a)$ such that $S_{a,b}(t_a,\bar{x}_0(t_a),\bar{y}_{t_a})=S_{a,b}(0)$. If we apply Clark-Ocone formula to $M(t_a,t_p)$ we get that
...
\end{align}
Now if we use (\ref{approximation_E_01_Ds_x_t}) and choose $\bar{x}_0(t_a)$ such that $S_{a,b}(t_a,\bar{x}_0(t_a),\bar{y}_{t_a})=S_{a,b}(0)$, we have that
\begin{align}
