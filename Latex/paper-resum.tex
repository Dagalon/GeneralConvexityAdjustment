...
\end{eqnarray}
with initial condition $\bar{x}_{0}(t_a)$ will be chosen appropriately depending on the case. In the appendix ($\cdots$), we can find the order of this approxiamtion.
...
\mathbb{E}_s^{t_p}\left(D_s \bar{x}_{t_a}\right) \eta(s,\bar{x}_s,\bar{y}_s) \exp\left(-\int_{s}^{t_a}k_u du \right) \mathbb{E}^{t_p}_s\left(\exp\left(-\int_{s}^{t_a} \exp\left(-\int_{u}^{t_a}k_u du\right)\partial_x (\eta(u, \bar{x}_u, \bar{y}_{u}) \bar{\nu}(u,t_p))|_{\bar{x}_u=\bar{x}_0(t_a)} du \right)\right).
\end{equation}
\end{lemma}
\begin{proof}
Using the relation 
...
\begin{equation*}
\mathbb{E}_s(D_s \bar{x}_{t_a}) \approx \eta(s,\bar{x}_s,\bar{y}_s) \exp\left(-\int_{s}^{t_a}k_u du \right) \mathbb{E}^{t_p}_s\left(\exp\left(-\int_{s}^{t_a} \exp\left(-\int_{u}^{t_a}k_u du\right)\partial_x (\eta(u, \bar{x}_u, \bar{y}_{u}) \bar{\nu}(u,t_p))|_{\bar{x}_u=\bar{x}_0(t_a)} du \right)\right) 
\end{equation*}
\end{proof}
