...
\begin{equation*}
x(t_a) = \int_{0}^{t_a} \exp\left(-\int_{u}^{t_a}k(w) dw\right) y(u) du + \int_{0}^{t_a}  \exp\left(-\int_{s}^{t_a}k(u) du \right) \eta(u,x(u),y(u)) dW_u^{\mathbb{Q}}. 
\end{equation*}
...
with 
\begin{equation*}
\beta(t,t_0,x,y) = \exp\left(-\int_{u}^{t_0}k(w) dw \right) \eta(u,x,y).
\end{equation*}
...
\begin{figure}[H]
	...
	\end{center}
	\caption{MonteCarlo Simulation vs Malliavin}
	\label{fig:Futures}
\end{figure} 
\end{example}
...
Given the Cheyette model in \eqref{short_rate_cheyette}, the hypotheses \ref{boundedness_volatility} and \ref{boundedness_reversion}, and considering the approximations in \eqref{approximation_y_t} and \eqref{approximation_x_t_a}. Given
\begin{equation}
