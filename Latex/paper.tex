...
\end{align*}
Now if we use again the Clark-Ocone representation, but in this case for $S_{a,b}(T_a)$, we can do the next approximation
\begin{align*}
...
\end{align*}
Where we have used that $S_{a,b}(t)$ and $S^{ois}_{a,b}(T_a)$ are $01(a,b)$-martingale. Also, if we use Itô formula we get that
\begin{align*}
...
\end{align*}
with initial condition respectivaly $S^{ois}_{a,b}(0)$ and $S_{a,b}(0)$. Then we have that
\begin{align*}
...
du
\end{align*}
Therefore, if se use the last equality we can approximate $CA_{CMS}(T_a,T_b)$ the next way
...
01(0,T_a,T_b)  f^{\prime}(S^{ois}_{a,b}(0)) \partial_{x}S_{a,b}(T_a) \partial_{x}S^{ois}_{a,b}(T_a) \frac{ \sigma^2(1-\exp(-2kT_a))}{2k} 
\end{equation}
...
\begin{align*}
CA_{CMS}(T_a,T_b) \\ 
\approx 01(0,T_a,T_b) \alpha \partial_{x}S_{a,b}(0) \partial_{x}S^{ois}_{a,b}(0) \frac{ \sigma^2(1-\exp(-2kT_a))}{2k}  \\   
...
\end{equation*}
In order to get an approximation for $D_sX_{T_a}$ we must to avoid the recurrence in the Malliavin derivative of $X_{T_a}$. Follow, the ideas of Piterrbarg Volume II, we can approximate $y_t$ the next way
\begin{equation*}
