...
\begin{align*}
f(S_{a,b}(T_a)) &= \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left(f(S_{a,b}(T_a))\right) + \int_{0}^{T_a} \mathbb{E}_s^{01(a,b)}\left( D_s (f(S_{a,b}(T_a))) \right) dW^{01(a,b)}_{s} \\
&= \frac{P_{ois}(0,T_p) }{01(0,T_a,T_b)} + \int_{0}^{T_a} \mathbb{E}_s^{01(a,b)}\left(f^{\prime}(S_{a,b}(T_a)) \partial_{x}S_{a,b}(T_a) D_s x_{T_a} \right) dW^{01(a,b)}_{s}. 
\end{align*}
Therefore, we can define the convexity adjustment for a CMS the next way
\begin{equation*}
CA_{CMS}(T_a,T_b) = 01(0,T_a,T_b) \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left(S_{a,b}(T_a) \int_{0}^{T_a} \mathbb{E}_s^{01(a,b)}\left( f^{\prime}(S_{a,b}(T_a)) \partial_{x}S_{a,b}(T_a) D_s x_{T_a} \right) dW^{01(a,b)}_{s} \right).
\end{equation*}
Now if we apply the adjoint of the Malliavin derivative (see Elisa-David) we get

\begin{align}
CA_{CMS}(T_a,T_b) &= 01(0,T_a,T_b) \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( \int_{0}^{T_a} \partial_{x}S_{a,b}(T_a) D_s x_{T_a} \mathbb{E}_s^{01(a,b)}\left(f^{\prime}(S_{a,b}(T_a)) \partial_{x}S_{a,b}(T_a) D_s x_{T_a} \right) ds \right) \nonumber \\
&\approx 01(0,T_a,T_b)  \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( \int_{0}^{T_a} \left(\partial_{x}S_{a,b}(0)\right)^{2} f^{\prime}(S_{a,b}(0))  D_s x_{T_a} \mathbb{E}_s^{01(a,b)}\left(D_s x_{T_a} \right) ds \right) \nonumber \\
& \approx 01(0,T_a,T_b)  \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( \int_{0}^{T_a} \left(\partial_{x}S_{a,b}(0)\right)^{2} f^{\prime}(S_{a,b}(0)) \beta(s,T_a,x_0,\hat{y}_{s})  D_s x_{T_a}  ds \right)
\end{align}
...
CA_{CMS}(T_a,T_b) \approx 01(0,T_a,T_b) \int_{0}^{T_a} \left(\partial_{x}S_{a,b}(0)\right)^{2} f^{\prime}(S_{a,b}(0)) \beta^{2}(s,T_a,x_0,\hat{y}_{u}) \Omega(s, T_a)    ds
\end{equation}
with 
