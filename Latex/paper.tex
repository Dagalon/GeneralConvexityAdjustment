...
The mean reversion function $k(\cdot)$ is positive a.s.
\end{hypothesis}
\begin{remark}
...
\begin{align*}
\mathbb{E}^{\mathbb{Q}}\left((A)\right) &\leq 16  t_a \alpha^{2}_2 C^{2}_{x,y} \int_{0}^{t_a} \exp\left(-2\int_{s}^{t_a}k_u^{\prime} du^{\prime}\right) G^{2}(u,t_a) \mathbb{E}^{\mathbb{Q}}\left(\|(\bar{x}_u -x_0,\bar{y}_u-y_0)\|^{2}\right) du \\
&+\leq 16  t_a \alpha^{2}_2 C^{2}_{x,y} \int_{0}^{t_a} \exp\left(-2\int_{s}^{t_a}k_u^{\prime} du^{\prime}\right) G^{2}(u,t_a) \mathbb{E}^{\mathbb{Q}}\left(\|(x_u - \bar{x}_u,y_u-\bar{y}_u)\|^{2}\right) du
\end{align*}
Again, from the hypotheses (\ref{boundedness_volatility}) and (\ref{boundedness_reversion}) we can show that 
\begin{equation*}
\mathbb{E}^{\mathbb{Q}}\left(\|(\bar{x}_u -x_0,\bar{y}_u-y_0)\|^{2}\right) = K h(u)
\end{equation*}
where $K$ is a constant with no dependency in $u$ and $h(u)\sim \mathcal{O}(u)$. In order to bound $(B)$, we will use the Doob's inequality and (\ref{boundedness_volatility}). Then
\begin{align*}
...
\begin{equation*}
\mathbb{E}^{\mathbb{Q}}\left(\|(x_{t_a} - \bar{x}_{t_a},y_{t_a} - \bar{y}_{t_a})\|^{2}\right) \leq C_1H(t_a) + C_2\int_{0}^{t_a} \mathbb{E}^{\mathbb{Q}}\left(\|(x_{u} - \bar{x}_{u},y_{u} - \bar{y}_{u})\|^{2}\right) du
\end{equation*}
where $H(t_a) \sim \mathcal{O}(t_a)$. To end, if we apply the Gronwall's lemma we have that
\begin{equation*}
\mathbb{E}^{\mathbb{Q}}\left(\|(x_{t_a} - \bar{x}_{t_a},y_{t_a} - \bar{y}_{t_a})\|^{2}\right) \leq C_1H(t_a) \exp\left(C_2 t_a\right)
\end{equation*}
