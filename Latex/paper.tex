...
\begin{align*}
a1 \leq \eta(t,x,y) \leq a2 \quad \forall (t,x,t) \in \mathbb{R}^{+} \times \mathbb{R} \times \mathbb{R}^{+}. \\
|\eta(t,x_2,y_2) - \eta(t,x_2,y_2)| \leq C_{x,y}\left(|x_2 - x_1| + |y_2 - y_1| \right) \quad \forall (t,x,t) \in \mathbb{R}^{+} \times \mathbb{R} \times \mathbb{R}^{+} 
\end{align*}
\end{hypothesis}
\begin{hypothesis}\label{boundedness_reversion} 
The next integrals are finite
\begin{align*}
I_{1,k}(0,t) &= \int_{0}^{t} \exp\left(- 2 \int_{s}^{t} k(s^{\prime})ds^{\prime}\right)  ds < \infty \\
I_{2,k}(0,t) &= \int_{0}^{t} \exp\left(- \int_{s}^{t} k(s^{\prime})ds^{\prime}\right) G(s,t) ds < \infty
\end{align*}
\end{hypothesis}
\begin{remark}
...
\section{Order of the approximation $\bar{y}_t$}
Under the assumption of $\eta(t,\cdot,\cdot)$ is global Lipschitz and with first derive continuous, we have that
\begin{align*}
|\eta^{2}(t,x_t,y_t) - \eta^{2}(t,x_0,y_0)|&=|\eta(t,x_t,y_t) - \eta(t,x_0,y_0)||\eta(t,x_t,y_t) + \eta(t,x_0,y_0)| \nonumber \\ 
&\leq C_{x,y}\left(|x_t - x_0| + |y_t - y_0|\right) + 
\end{align*}
