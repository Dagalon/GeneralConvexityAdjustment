...
\begin{example}
In order to get a little bit of intuition we will use the las t expression for the particular case of a Hull-White model. In this case, is easy to show that 
$$
D_s x_{T_a} = \sigma \exp(-(T_a - s))
$$
therefore we have that 
\begin{equation*}
\mathbb{E}^{T_p}\left(S_{a,b}(T_a)\right) = S_{a,b}(0) + \frac{1}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a} (D_s X_{T_a})^{2}  \mathbb{E}^{0,1}_s\left(\partial_x S_{a,b}(s) \partial_x M(s,T_p)  \right) ds \right)
\end{equation*}
Now, if we apply again Clark-Ocone to $\partial_x M(s,T_p)$, we get that

\begin{align*}
\mathbb{E}^{T_p}\left(S_{a,b}(T_a)\right) &= S_{a,b}(0) + \frac{\partial_x S_{a,b}(0) \partial_x M(0,T_p)}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a} (D_s X_{T_a})^{2} ds \right) \\
&+ \frac{1}{M(0,T_p)} \mathbb{E}^{01}\left(\partial_x S_{a,b}(s) \int_{0}^{T_a} (D_s X_{T_a})^{2} \int_{0}^{s} \partial^{2}_{x} S_{a,b}(u_1) \partial^{2}_{x} M(u_1,T_p) (D_{u_1}x_s)^{2}  du_1 ds \right)
\end{align*}
We must observe that, if we repeat the same argument and  
...
\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.3]{Figures/cms_convexity_order_1.jpg}
		\caption{Convexity Mc Vs Convexity Malliavin}
	\end{center}
\end{figure} 
...
\section{Approximation of $\mathbb{E}^{01}\left(x_t\right)$ and $\mathbb{E}^{01}\left(\left(x_t -\mathbb{E}^{01}\left(x_t\right)\right)^{2}\right)$}
...
Therefore, we have that
\begin{align}
\mathbb{E}^{01}\left(x_t\right) &\approx \bar{x}_0  + \int_{0}^{t} \exp\left(-\int_{s}^{t}k_u du\right) \bar{y}(u) du + \nonumber \\ &\int_{0}^{t} \exp\left(-\int_{s}^{t}k_u du \right) \eta(u,\bar{x}(u),\bar{y}(u)) \sigma_{01}(u, T_a, T_b) du \nonumber \\
\mathbb{E}^{01}\left(\left(x_t -\mathbb{E}^{01}\left(x_t\right)\right)^{2}\right) &\approx \int_{0}^{t} \exp\left(- 2 \int_{s}^{t}k_u du \right) \eta^{2}(u,\bar{x}(u),\bar{y}(u)) du
\end{align}
