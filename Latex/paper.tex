...
\end{equation}
where $M(t,T_p)= \frac{P_{ois}(t,T_p)}{01(t,T_a,T_p)}$. Now ¡, if we apply Clark-Ocone formula to $S_{a,b}(T_a)$ and $M(T_a,T_p)$ we get
\begin{align} \label{clark_ocone_swap_m}
S_{a,b}(T_a) &= S_{a,b}(0) + \int_{0}^{T_a} \mathbb{E}_s^{01}\left(D_s S_{a,b}(T_a) \right) dW^{01}_s  \nonumber \\
...
\begin{align}
\mathbb{E}^{T_p}\left(S_{a,b}(T_a)\right) &= S_{a,b}(0) + \frac{1}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a} \mathbb{E}^{01}_{s}\left(\partial_x S_{a,b}(T_a)  D_s x_{T_a} \right)
\mathbb{E}^{01}_{s}\left(\partial_x M(T_a,T_p)  D_s x_{T_a} \right) ds \right) \nonumber \\
&= S_{a,b}(0) + \frac{\partial_x S_{a,b}(0)\partial_x M(0,T_p)}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a}  \left(\mathbb{E}^{01}\left(D_s x_{T_a}\right)\right)^{2}
 ds\right) \nonumber \\
&+ \cdots
\end{align}

\begin{example}
In the case of a Hull-White model, we showed  in 
$$
D_s x_{T_a} = \sigma \exp(-(T_a - s))
$$
and therefore, we have that
\begin{equation*}
\mathbb{E}^{\mathbb{Q}^{01}}\left(\int_{0}^{T_a}(D_s x_{T_a})^{2} ds \right) = \sigma^{2} \frac{1 - \exp(-2kT_a)}{2k}.
\end{equation*}
