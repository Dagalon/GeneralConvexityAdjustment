...
\eqref{forward_rate} and \eqref{future}, we can express the convexity adjustment definition as:%From \eqref{forward_rate} and \eqref{future}, we will define the convexity adjustment as
\begin{equation*}
...
\end{equation*}
We suppose, that we have a flow in $T_a < T_p < T_b$ with value $S_{a,b}(T_a)$. We know that, $S_{a,b}(T_a)$ is a martingale under the measure $\mathbb{Q}^{01}$, but not under the measure $\mathbb{Q}^{T_p}$. Therefore
\begin{equation*}
\mathbb{E}^{T_p}\left(S_{a,b}(T_a)\right) \neq S_{a,b}(0).
\end{equation*}
Then convexity adjustment for a CMS will be define as
\begin{equation}
CA_{CMS}(T_p) = \mathbb{E}^{T_p}\left(S_{a,b}(T_a)\right) - S_{a,b}(0).
\end{equation} 
Now, after some changes of measure, it is easy to show that
\begin{equation}\label{expected_t_p_cms}
...
\begin{example}
In order to get a little bit of intuition we will use the last expression for the particular case of a Hull-White model. In this case, is easy to show that 
$$
D_s x_{T_a} = \sigma \exp(-(T_a - s))
$$
therefore we have that 
\begin{equation*}
\mathbb{E}^{T_p}\left(S_{a,b}(T_a)\right) = S_{a,b}(0) + \frac{1}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a} (D_s X_{T_a})^{2} \partial_x S_{a,b}(s) \partial_x M(s,T_p) ds \right)
\end{equation*}
Now, if we apply again Clark-Ocone to $\partial_x M(s,T_p)$, we get that

\begin{align*}
\mathbb{E}^{T_p}\left(S_{a,b}(T_a)\right) &= S_{a,b}(0) + \frac{\partial_x S_{a,b}(0) \partial_x M(0,T_p)}{M(0,T_p)}\int_{0}^{T_a} (D_s X_{T_a})^{2} ds \\
&+ \frac{1}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a} (D_s X_{T_a})^{2} \int_{0}^{s} \partial^{2}_{x} S_{a,b}(u_1) \partial^{2}_{x} M(u_1,T_p) (D_{u_1}x_s)^{2}  du_1 ds \right)
\end{align*}
We must observe that, that it is easy to get an extra term in the expansion, only we must repeat the same argument that before, but now in $\partial^{2}_x M(u_1, T_p)$ we will get the next approximation

\begin{align*}
\mathbb{E}^{T_p}\left(S_{a,b}(T_a)\right) &\approx S_{a,b}(0) + \frac{\partial_x S_{a,b}(0) \partial_x M(0,T_p)}{M(0,T_p)} \int_{0}^{T_a} (D_s X_{T_a})^{2} ds \\
&+ \frac{\partial^{2}_{x} S_{a,b}(0) \partial^{2}_{x} M(0,T_p)}{M(0,T_p)} \int_{0}^{T_a} (D_s X_{T_a})^{2} \int_{0}^{s}  (D_{u_1}x_s)^{2}  du_1 ds
\end{align*}
In the next figure, we can see the convexity adjustment for a CMS where the tenor for the underlying swap is 5Y using the firt approximation and Monte Carlo method
\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.25]{Figures/cms_convexity_order_1.jpg}
		\caption{Convexity Mc Vs Convexity Malliavin}
	\end{center}
\end{figure} 
...
\end{example}
Now, we will continue with the generic case. Follow the same idea, that in the last example, if we apply Clark-Ocone to $\partial_x M(T_a,T_p)$ we have that
\begin{equation*}
\partial_x M(T_a,T_p) = \partial_x M(0,T_p) + \int_{0}^{T_a} \mathbb{E}_{u_1}^{01}\left(D_{u_1} \partial^{2}_x M(T_a,T_p) D_{u_1} x_{T_a} \right) dW^{01}_{u_1}  
\end{equation*}
Therefore
\begin{align}\label{general_cms_approximatìon_first_step}
\mathbb{E}^{T_p}\left(S_{a,b}(T_a)\right) &= S_{a,b}(0) + \frac{\partial_x M(0,T_p)}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a} D_s X_{T_a} \partial_x S_{a,b}(T_a)
\mathbb{E}^{01}_s\left(D_s x_{T_a} \right) ds\right) \\
&+ \frac{1}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a} \partial_x S_{a,b}(T_a) D_{s} X_{T_a} \int_{0}^{T_a} \mathbb{E}_{u_1}^{01}\left(D_{u_1} \partial^{2}_x M(T_a,T_p) D_{u_1} x_{T_a} \right) dW^{01}_{u_1} ds \right)
\end{align}
Let us define
\begin{align*}
A(T_a) &= \frac{\partial_x M(0,T_p)}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a} D_s X_{T_a} \partial_x S_{a,b}(T_a) \mathbb{E}^{01}_s\left(D_s x_{T_a} \right) ds\right) \\
\epsilon(T_a) &= \frac{1}{M(0,T_p)} \mathbb{E}^{01}\left(\int_{0}^{T_a} \partial_x S_{a,b}(T_a) D_{s} X_{T_a} \int_{0}^{T_a} \mathbb{E}_{u_1}^{01}\left(D_{u_1} \partial^{2}_x M(T_a,T_p) D_{u_1} x_{T_a} \right) dW^{01}_{u_1} ds \right)
\end{align*}
If we use again Calrk-Ocone with $\partial_x S_{a,b}(T_a)$ we have that
\begin{equation*}
\partial_x S_{a,b}(T_a) = \partial_x S_{a,b}(0) + \int_{0}^{T_a} \mathbb{E}^{01}\left( \mathbb{E}^{01}\left( \partial^{2}_x S_{a,b}(T_a)\right) D_s X_{T_a}  dW^{01}_s  \right) 
\end{equation*}
Therefore
\begin{equation*}
A(T_a) =
...
\end{equation}
With initial condition  $\bar{x}_{0}(T_a)$ is such that $S_{a,b}(0,\bar{x}_0(T_a), \bar{y}({T_a})) = S_{a,b}(0)$ (see Piterbarg Volume II). Now, we must remember that by Girsanov's theorem we have that 
\begin{equation*}
...
\mathbb{E}_s^{T_p}\left(D_s x_{T_a}\right) \approx \eta(s,\bar{x_0(s)},\bar{y}_{s}) \exp\left(-\int_{s}^{T_a}k_u du \right) \exp\left(-\int_{s}^{T_a} \exp\left(-\int_{u}^{T_a}k_u du\right)\partial_x (\bar{\nu}(u,T_a) \bar{\nu}(u,T_p)) du \right).
\end{equation}
