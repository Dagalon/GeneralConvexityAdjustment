...
\mathbb{E}^{\mathbb{Q}^{T_{a}}}\left(S_{a,b}(T_a) P_{ois}(T_a,T_p) \right) &=  \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left(S_{a,b}(T_a) \frac{M(T_a)}{M(0)}\right) \nonumber \\
& = S_{a,b}(0) + \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left(S_{a,b}(T_a) \left(\frac{M(0, T_p)}{M(T_a, T_p)} - 1\right) \right)  
\end{align} 
where $M(t)= \frac{01(t,T_a,T_b)}{P_{ois}(t,T_p)} $. From (\ref{cms_expedted_value}), we will define the convexity adjusment for a CMS the next way
\begin{equation}\label{cms_convexity}
CA_{CMS}(T_a,T_b) = \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left(S_{a,b}(T_a) \left(\frac{M(0, T_p)}{M(T_a, T_p)} - 1\right) \right). 
\end{equation}
In order to reduce the complexity to explain the movement od the forward curve, it is a common pratice to asumme that
\begin{equation*}
M(t) = f(S_{a,b}(t))
\end{equation*}
where $f(\cdot)$ is known as mapping function. There is a extensive literature how to define this function (see Piterbarg Volume III). Then under this assumption and applying Clark-Ocone to $f(S_{a,b}(t))$ we get
\begin{align*}
f(S_{a,b}(T_a)) &= \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left(S_{a,b}(T_a)\right) + \int_{0}^{T_a} \mathbb{E}_s^{01(a,b)}\left( D_s (f(S_{a,b}(T_a))) \right) dW^{01(a,b)}_{s} \\
&= 1 + \int_{0}^{T_a} \mathbb{E}_s^{01(a,b)}\left(f^{\prime}(S_{a,b}(T_a)) \partial_{x}S_{a,b}(T_a) D_s x_{T_a} \right) dW^{01(a,b)}_{s}. 
\end{align*}
Therefore, we have that
\begin{equation*}
CA_{CMS}(T_a,T_b) = \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left(S_{a,b}(T_a) \int_{0}^{T_a} \mathbb{E}_s^{01(a,b)}\left( f^{\prime}(S_{a,b}(T_a)) \partial_{x}S_{a,b}(T_a) D_s x_{T_a} \right) dW^{01(a,b)}_{s} \right).
\end{equation*}
Now if we apply the adjoint of the Malliavin derivative (see Elisa-David) we get

\begin{align}
CA_{CMS}(T_a,T_b) &= \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( \int_{0}^{T_a} \partial_{x}S_{a,b}(T_a) D_s x_{T_a} \mathbb{E}_s^{01(a,b)}\left(f^{\prime}(S_{a,b}(T_a)) \partial_{x}S_{a,b}(T_a) D_s x_{T_a} \right) ds \right) \nonumber \\
&\approx  \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( \int_{0}^{T_a} \left(\partial_{x}S_{a,b}(0)\right)^{2} f^{\prime}(S_{a,b}(0))  D_s x_{T_a} \mathbb{E}_s^{01(a,b)}\left(D_s x_{T_a} \right) ds \right) \nonumber \\
& \approx \mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( \int_{0}^{T_a} \left(\partial_{x}S_{a,b}(0)\right)^{2} f^{\prime}(S_{a,b}(0)) \beta(s,T_a,x_0,\hat{y}_{s})  D_s x_{T_a}  ds \right)
\end{align}
...
\begin{equation*}
\mathbb{E}^{\mathbb{Q}^{01(a,b)}}\left( D_s x_{T_a} \right) \approx \beta(s,T_a,x_0,\hat{y}_{s}) \exp\left(\int_{s}^{T_a} \frac{\int_{T_a}^{T_b} P_{ois}(u,u^{\prime},x_0,\hat{y}_u) \nu(t,u^{\prime}) du^{\prime}}{01^{c}(u,T_a,T_b,x_0,\hat{y}_u)} \partial_x \beta(u,t_0,x_s,y_s) du \right)
\end{equation*}
Therefore we have the next approximation for $CA_{CMS}(T_a,T_b)$

\fboxsep1em
\fbox{\parbox{465pt}
{
\begin{equation*}
\int_{0}^{T_a} \left(\partial_{x}S_{a,b}(0)\right)^{2} f^{\prime}(S_{a,b}(0)) \beta^{2}(s,T_a,x_0,\hat{y}_{s})  \exp\left(\int_{s}^{T_a} \frac{\int_{T_a}^{T_b} P_{ois}(u,u^{\prime},x_0,\hat{y}_u) \nu(t,u^{\prime}) du^{\prime}}{01^{c}(u,T_a,T_b,x_0,\hat{y}_u)} \partial_x \beta(u,t_0,x_s,y_s) du \right)  ds
\end{equation*}
}}
