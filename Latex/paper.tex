...
\end{equation*}
In the next figure, we can see the convexity adjustment for a CMS where the tenor of the underlying swap is 5Y using the last approximation and Montecarlo method with parameters for the Hull-White model $\sigma=0.007$ and $k=0.0007$
...
\begin{equation}\label{y_t_approximation}
\bar{y}(t)= \int_{0}^{s} \exp\left(-2\int_{u}^{s} k_{u^{\prime}} du^{\prime} \right) \eta(u,x_0,y_0), 
\end{equation}
we have that
...
\begin{equation}\label{approximation_E_s_x_t}
...
\begin{equation}\label{approximation_x_t_a}
X_{T_a} \approx \bar{X}_{T_a}:= \bar{x}_0  + \int_{0}^{T_a} \exp\left(-\int_{s}^{T_a}k_u du\right) \bar{y}(u) du + \int_{0}^{T_a}  \exp\left(-\int_{s}^{T_a}k_u du \right) \eta(u,\bar{x}(u),\bar{y}(u)) dW_u^{\mathbb{Q}}  
\end{equation}
...
\sigma_{01}(t,T_a,T_b) \approx \bar{\sigma}_{0,1}(t,T_a,T_b):= \sum_{i=a+1}^{b} w_i \nu(t,T_i)
\end{equation}
